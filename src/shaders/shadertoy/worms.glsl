...
vec3 doColor( in vec3 sp, in vec3 rd, in vec3 sn, in vec3 lp, vec2 mat) {
	vec3 ld = lp - sp;	// -light_direction
    float lDist = max(length(ld), 0.001);
    ld /= lDist; //	normalize

    float atten = 2.0 / (1.0 + lDist * 0.525 + lDist * lDist * 0.05);	// attenuation
	float diff = max(dot(sn, ld), .1);	// diffuse; ambient = 0.1
    float spec = pow(max(dot(reflect(-ld, sn), -rd), 0.0), 1.0);	// phong
